...
dbFollowerManager = Annotated[DBManager, Depends(DBHostAdapter(DBHost.FOLLOWER))]
...
async def get_current_user_or_none(
    ...
    if request.headers.get("Authorization"):
        ...
        try:
            user = await get_current_user(token=token, db_manager=db_manager)
...
async def verify_google_token(db_manager: dbManager, token: str) -> dict:
    """
    Verify Google authentication token.

    Parameters
    ----------
    token - Google auth token to verify

    Returns
    -------
    User info if token is valid, raises HTTPException if invalid
    """
    ...
    async with AsyncClient() as client:
        ...
        if user_info_response.status_code != 200:
            logger.error(
                f"Token verification failed: {user_info_response.json()}"
...
async def get_current_user_from_multiple_auth(
    ...
    if api_key:
        try:
            return await verify_google_token(db_manager=db_manager, token=api_key)
    ...
    if token:
        try:
            return await get_current_user(token=token, db_manager=db_manager)
        except HTTPException:
            pass
