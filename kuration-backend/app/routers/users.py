...
async def register_user(db_manager: dbManager, user: UserCreate) -> UserCreateResponse:
    """
    Register a new user.
    ...
    async with db_manager.get_session() as _session:
        ...
        new_user.password = history_pass
...
def get_meta_content(soup, name):
    """Helper function to get meta content by name."""
    meta = soup.find("meta", {"name": name})
    return meta["content"] if meta else None


def extract_email(soup):
    """Extract email addresses from the soup using regex."""
...
def extract_phone(soup):
    """Extract phone numbers from the soup using regex."""
...
async def scrape_lead_data(lead: LeadRequest):
    try:
        # Fetch the webpage
        async with httpx.AsyncClient() as client:
            response = await client.get(lead.website_url)
        ...
        # Add fields only if they are found
        industry = get_meta_content(soup, "industry")
        if industry:
        ...
        if email:
            company_info["contact_email"] = email
