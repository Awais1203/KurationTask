...
class ErrorMessage(str, Enum):
    ...
    PASSWORD_DOES_NOT_MATCH = (
        "The current password does not match with the password from the" " database."
    )
...
async def load_permission(permission_id: int, session: AsyncSession) -> Permission:
    """
    ...
    if permission is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail={"permission_id": ErrorMessage.PERMISSION_NOT_FOUND_MESSAGE},
        )
...
async def user_existence(user_email: str, session: AsyncSession):
    ...
    # load user
    result = await session.execute(
        select(User).options(selectinload(User.groups)).where(User.email == user_email)
    )
    user = result.scalars().first()
    if user is None:
        raise HTTPException(
...
async def check_password_history(
    ...
    password_history_list = await retrieve_user_password_history(user_id, session)
...
async def send_email(subject, verification_token, email):
    try:
        ...
        body = f"Click the following link to verify your email: http://127.0.0.1:8000/users/email/verification?email={email}&token={verification_token}"
